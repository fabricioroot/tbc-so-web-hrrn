...
public class MainScreen extends javax.swing.JApplet {
    ...
    private void initComponents() {
        ...
        jPanelIntroductionLayout.setHorizontalGroup(
            ...
            .addComponent(jScrollPaneIntroduction, javax.swing.GroupLayout.DEFAULT_SIZE, 1038, Short.MAX_VALUE)
        );
                        ...
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            ...
                            .addGroup(layout.createSequentialGroup()
                                ...
                                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                    ...
                                    .addComponent(jPanelAnimation, javax.swing.GroupLayout.DEFAULT_SIZE, 627, Short.MAX_VALUE)))))
                    .addGroup(layout.createSequentialGroup()
                        .addGap(245, 245, 245)
    ...
    private void jButtonIntroductionActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonIntroductionActionPerformed
        final String TEXT_BASE_INTRODUCTION = "\n" +
                                              "Informalmente, processo pode ser visto como um programa em execução, porém esta é uma idéia incompleta de como o sistema operacional o trata.\n" +
                                              "Atualmente, os computadores são capazes de realizar várias tarefas ao mesmo tempo. Ou seja, equanto executa um programa, um computador pode,\n" +
                                              "por exemplo, executar operações de leitura/escrita no disco ou executar outro programa. Em sistemas monoprocessados, as tarefas não são executadas\n" +
                                              "ao mesmo tempo, existe apenas a impressão delas estarem sendo executadas ao mesmo tempo, pois o processador é capaz de realizar um tarefa por vez.\n" +
                                              "Nesse sentido, existe o conceito de multiprogramação, que é a capacidade de armazenar o código de muitos processos (programas) na memória\n" +
                                              "simultaneamente para que eles possam ser executados. Assim, os programas podem acessar os recursos do computador e serem executados em um tempo\n" +
                                              "razoável tendo a sensação de multiprocessamento.\n\n" +
                                              "Nesse contexto, basicamente o gerenciador de processos garante que cada processo receba uma fatia de tempo suficiente para funcionar corretamente e\n" +
                                              "escalona (de acordo com a política implementada) os processos presentes na fila de prontos para que eles sejam executados. A tarefa dessas políticas\n" +
                                              "é escolher qual dos processos presentes na fila de prontos será o próximo a ser executado. Utiliza-se como critério básico o objetivo de aumentar ao\n" +
                                              "máximo o rendimento do sistema operacional, diminuido o tempo de resposta (turnaround time = intervalo de tempo da submissão de um processo até o seu\n" +
                                              "termino) ao usuário. Para conseguir um aumento de eficiência no processador, busca-se reduzir o tempo médio de espera na fila de processos prontos.\n" +
                                              "Tempo de espera é a soma dos períodos gastos por um processo aguardando na fila de espera.\n\n" +
                                              "Existem duas categorias principais de políticas de escalonamento: preemptiva e não-preemptiva. Nas políticas não-preemptivas, quando um processo assume\n" +
                                              "o controle do processador, ele o mantém até terminar. Sua principal vantagem em relação às políticas preemptivas, é a simplicidade. Sua desvantagem é\n" +
                                              "a falta de resposta ao comportamento do sistema quando, por exemplo, um processo com alto tempo de processamento detém controle do processador, todos\n" +
                                              "os demais têm de esperar. Nas políticas preemptivas, depois de certo tempo (devido, por exemplo, às diferentes prioridades dos processos), o sistema\n" +
                                              "operacional pode retomar o controle do processador.\n\n" +
                                              "*Este software educativo trata da política HRRN (Highest Response Ratio Next - Próxima taxa de resposta mais alta).\n" +
                                              "Esta política tenta resolver algumas deficiências da política SJF (Shortest Job First), especificamente o viés excessivo contra processos demorados e o\n" +
                                              "favoritismo excessivo em relação a processos pequenos. Esta é um política não-preemptiva na qual define-se dinamicamente a prioridade de cada processo\n" +
                                              "a partir da fórmula:\n\n" +
                                              "Prioridade = tempo de espera + tempo de vida\n" +
                                              "                    ------------------------------------------\n" +
                                              "                                 tempo de vida\n\n" +
                                              "Neste software o tempo de vida é igual ao tempo de burst do processo.\n" +
                                              "Analisando a fórmula, com o tempo de vida no denominador, processos que levam menor tempo para terminar recebem prefência. Entretanto, com o tempo de\n" +
                                              "espera no numerador, processos que levam maior tempo para terminar recebem tratamento preferencial.\n" +
                                              "Assim, cada vez que o botão \"Passos do Algoritmo\" é clicado, esta conta é feita e é escalonado aquele processo de maior prioridade, impedindo o\n" +
                                              "adiamento da execução de processos grandes indefinidamente.\n\n" +
                                              "* VEJA O ALGORITMO ABAIXO";
        final String TEXT_BASE_ALGORITHM = "funcao hrrn (referência de vetor<TAD_Processo> listaDeProntos) : TAD_Processo\n" +
                                           "inicio\n" +
                                           "    TAD_Processo processo;\n" +
                                           "    inteiro aux := 0;\n\n" +
                                           "    se (listaDeProntos não está vazia)\n" +
                                           "        listaDeProntos := calculaPrioridades(listaDeProntos);\n" +
                                           ...
                                           "funcao calculaPrioridades(vetor<TAD_Processo> listaDeProcessos) : vetor<TAD_Processo>\n" +
                                           "inicio\n" +
                                           "    vetor<TAD_Processo> saida := listaDeProcessos;\n" +
                                           "    inteiro aux := 0;\n\n" +
                                           "    para (inteiro i := 0; i <= (tamanho de listaDeProcessos - 1); i++)\n" +
                                           "        aux := 1 + (listaDeProcessos.elemento(i).tempoDeEspera / listaDeProcessos.elemento(i).tempoDeBurst);\n" +
                                           "        saida.elemento(i).prioridade := aux;\n" +
                                           "    Fim para\n" +
                                           "    calculaPrioridades := saida;\n" +
                                           "fim.";
    ...
    private void jButtonAlgorithmStepsActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonAlgorithmStepsActionPerformed
        if(st != null) {
            ...
            this.processesList = this.st.getProcessesList();
        }
    ...
    public void paintProcessesList(Vector<Process> processesQueue) {
        ...
        for(int i = 0; i <= (processesQueue.size() - 1); i++) {
            ...
            block.setToolTipText("Tempo de burst = " + String.valueOf((int)processesQueue.elementAt(i).getLifeTime()) + ";   Tempo na criação = " + String.valueOf((int)processesQueue.elementAt(i).getCreationTime()) + ";   Tempo de espera = " + String.valueOf((int)processesQueue.elementAt(i).getWaitingTime()) + ";   Prioridade = " + df.format(processesQueue.elementAt(i).getPriority()));
